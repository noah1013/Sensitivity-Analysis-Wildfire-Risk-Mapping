...
average_climate_data <- function(fire_season,
  ...
  climate_data_path <- paste0(dirname(path), "/Data/Intermediate/climate_data/SSP",
  ...
  for (variable in climate_data) {
    ...
    yearly_vars <- list.files(variable,
                              full.names = TRUE)
    ...
    for (key in names(fire_season)) {
      ...
      for (data in yearly_vars){
        ...
        if (basename(data) == key){
          ...
          # list the files
          monthly_agg_list <- list.files(data,
                                         full.names = TRUE)
  ...
  # save the file.
  cmip6_fire_season_output <- paste0(dirname(path), "/Data/Intermediate/fs_climate_data")
...
cmip6_dir <- file.path(dirname(path), "Data/Initial/climate_data") 

# there are 6 variables for 2 different (2 and 5) SSP scenarios:

# hurs - near-surface relative humidity
# pr - total precipitation
# tas - near-sruface air temperature
...
output_folder <- paste0(dirname(path), "/Data/Intermediate/climate_data/")
...
# import the data:
fire_months <- readLines(paste0(dirname(path), 
                                "/Data/stats/yearly_fire_seasons/fire_seasons.txt"))
...
future_fire_season <- list('2080' = c("06", "07","08", "09", "10"),
